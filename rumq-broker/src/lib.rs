...
async fn tls_connection<P: AsRef<Path>>(ca_path: P, key_path: P) -> Result<TlsAcceptor, Error> {
    let mut server_config = ServerConfig::new(NoClientAuth::new());

    let certs = certs(&mut BufReader::new(File::open(ca_path)?)).map_err(|_|Error::CertFail)?;
    let mut keys = rsa_private_keys(&mut BufReader::new(File::open(key_path)?)).map_err(|_| Error::KeyFail)?;
...
pub async fn accept_loop(config: Arc<Config>, addr: &str) -> Result<(), Error> {
    ...
    let acceptor = if let Some(ca_path) = config.ca_path.as_ref() {
        let key_path = config.key_path.as_ref().ok_or(Error::NoPrivateKey)?;
        Some(tls_connection(ca_path, key_path).await?)
    } else {
        None
    };
