...
-   if `Router::readyqueue` is empty, blocks and wait over events receiver to prevent spinning in loop. There is nothing to send over to `RemoteLink`, and thus blocking here is fine.
...
For each filter subscribed to, we validate the subscription and call `Router::prepare_consumption()`, which adds connection to readyqueue. If subscription is invalid, connection is disconnected.

#### PubAck Packets

These packets are registered with the QoS1 buffer in `Connection`, and if the ack is out of order or unsolicited then we disconnect.
...
1. Broker
    ...
    1. The Router is started, and we receive a channel (`router_tx`), which all other Links and Servers pass `Event`s into
...
2. Router
    ...
    Once all necessary things have been initialized, the `spawn` method initializes the router's event loop on a separate thread.
...
-   `server` -> Contains the Broker, TLS Connection Logic, and some Common Types
