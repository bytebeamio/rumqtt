...
impl Broker {
    ...
    pub fn link(&self, client_id: &str) -> Result<(LinkTx, LinkRx), LinkError> {
    ...
    pub fn start(&mut self) -> Result<(), Error> {
        ...
        if let Some(metrics_config) = self.config.metrics.clone() {
            let timer_thread = std::thread::Builder::new().name("timer".to_owned());
        ...
        if let Some(bridge_config) = self.config.bridge.clone() {
            let bridge_thread = std::thread::Builder::new().name(bridge_config.name.clone());
            let router_tx = self.router_tx.clone();
        ...
        if let Some(v4_config) = &self.config.v4 {
            for (_, config) in v4_config.clone() {
                let server_thread = std::thread::Builder::new().name(config.name.clone());
        ...
        if let Some(prometheus_setting) = &self.config.prometheus {
            ...
            let metrics_thread = std::thread::Builder::new().name("Metrics".to_owned());
        ...
        if let Some(console) = self.config.console.clone() {
            ...
            let console_thread = std::thread::Builder::new().name("Console".to_string());
