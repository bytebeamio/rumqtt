...
pub enum StateError {
    ...
    InvalidState,
    /// Received a packet with pkid 0
    #[error("Pkid must be non-zero")]
    ZeroPkid,
...
impl MqttState {
    ...
    pub fn clean(&mut self) -> Vec<Request> {
        ...
        // NOTE(swanx): why not just call self.incoming_pub.clear()??
...
mod test {
    ...
    // we don't increment pkid here, so this won't work!
    // #[test]
