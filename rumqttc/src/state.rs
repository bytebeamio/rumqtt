use crate::{Event, Incoming, Outgoing, Pending, PromiseTx, Request};
...
pub struct MqttState {
    ...
    pub collision: Option<Pending<Publish>>,
...
impl MqttState {
    ...
    pub fn handle_outgoing_packet(
        ...
        let packet = match request {
            Request::Publish(publish) => self.outgoing_publish(Pending {
                request: publish,
                promise_tx,
            })?,
            Request::PubRel(pubrel) => self.outgoing_pubrel(pubrel)?,
            Request::Subscribe(subscribe) => self.outgoing_subscribe(Pending {
                request: subscribe,
                promise_tx,
            })?,
            Request::Unsubscribe(unsubscribe) => self.outgoing_unsubscribe(Pending {
                request: unsubscribe,
                promise_tx,
            })?,
    ...
    fn handle_incoming_puback(&mut self, puback: &PubAck) -> Result<Option<Packet>, StateError> {
        ...
        self.inflight -= 1;
        let packet = self.check_collision(puback.pkid).map(
            |Pending {
                 request: publish,
                 promise_tx: tx,
             }| {
                ...
                self.collision_ping_count = 0;
                self.ack_waiter[puback.pkid as usize] = tx;
    ...
    fn handle_incoming_pubcomp(&mut self, pubcomp: &PubComp) -> Result<Option<Packet>, StateError> {
        ...
        self.inflight -= 1;
        let packet = self.check_collision(pubcomp.pkid).map(
            |Pending {
                 request: publish,
                 promise_tx: tx,
             }| {
                ...
                self.events.push_back(event);
                self.collision_ping_count = 0;
                self.ack_waiter[pubcomp.pkid as usize] = tx;
    ...
    fn outgoing_subscribe(
        ...
        self.events.push_back(event);
        self.ack_waiter[subscription.pkid as usize] = tx;
    ...
    fn outgoing_unsubscribe(
        ...
        self.events.push_back(event);
        self.ack_waiter[unsub.pkid as usize] = tx;
    ...
    fn check_collision(&mut self, pkid: u16) -> Option<Pending<Publish>> {
        match &self.collision {
            Some(Pending { request, .. }) if request.pkid == pkid => self.collision.take(),
            _ => None,
        }
    }
...
mod test {
    ...
    fn outgoing_publish_should_set_pkid_and_add_publish_to_queue() {
        ...
        mqtt.outgoing_publish(Pending::no_promises(publish))
            .unwrap();
        ...
        mqtt.outgoing_publish(Pending::no_promises(publish.clone()))
            .unwrap();
    ...
    fn incoming_puback_should_remove_correct_publish_from_queue() {
        ...
        mqtt.outgoing_publish(Pending::no_promises(publish1))
            .unwrap();
        mqtt.outgoing_publish(Pending::no_promises(publish2))
            .unwrap();
        assert_eq!(mqtt.inflight, 2);
    ...
    fn incoming_pubrec_should_release_publish_from_queue_and_add_relid_to_rel_queue() {
        ...
        let _publish_out = mqtt.outgoing_publish(Pending::no_promises(publish1));
        let _publish_out = mqtt.outgoing_publish(Pending::no_promises(publish2));
    ...
    fn incoming_pubrec_should_send_release_to_network_and_nothing_to_user() {
        ...
        let publish = build_outgoing_publish(QoS::ExactlyOnce);
        let packet = mqtt
            .outgoing_publish(Pending::no_promises(publish))
            .unwrap()
            .unwrap();
        match packet {
    ...
    fn clean_is_calculating_pending_correctly() {
        ...
        let res = vec![6, 1, 2, 3];
        for (Pending { request, .. }, idx) in pending.iter().zip(res) {
