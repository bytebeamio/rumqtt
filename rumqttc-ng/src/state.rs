...
impl MqttState {
    ...
    pub fn clean(&mut self) -> Vec<Request> {
        ...
        for pkid in self.outgoing_rel.ones() {
            let request = Request::PubRel(PubRel {
                pkid: pkid as u16,
...
mod test {
    ...
    fn build_puback(pkid: u16) -> PubAck {
        PubAck {
            pkid,
            reason: PubAckReason::Success,
            properties: None,
    ...
    fn build_pubrec(pkid: u16) -> PubRec {
        PubRec {
            pkid,
            reason: PubRecReason::Success,
            properties: None,
    ...
    fn build_pubrel(pkid: u16) -> PubRel {
        PubRel {
            pkid,
            reason: PubRelReason::Success,
            properties: None,
    ...
    fn build_pubcomp(pkid: u16) -> PubComp {
        PubComp {
            pkid,
            reason: PubCompReason::Success,
            properties: None,
    ...
    fn incoming_publish_should_be_acked() {
        ...
        mqtt.handle_incoming_publish(&publish1).unwrap();
        let packet = mqtt.handle_incoming_publish(&publish2).unwrap().unwrap();
        match packet {
            Packet::PubAck(puback) => assert_eq!(puback.pkid, 2),
            _ => panic!("Expected PubAck"),
        }

        let packet = mqtt.handle_incoming_publish(&publish3).unwrap().unwrap();
        match packet {
            Packet::PubRec(pubrec) => assert_eq!(pubrec.pkid, 3),
    ...
    fn incoming_puback_should_remove_correct_publish_from_queue() {
        ...
        mqtt.handle_incoming_puback(&build_puback(1)).unwrap();
        assert_eq!(mqtt.inflight, 1);

        mqtt.handle_incoming_puback(&build_puback(2)).unwrap();
        assert_eq!(mqtt.inflight, 0);
    ...
    fn incoming_puback_with_pkid_greater_than_max_inflight_should_be_handled_gracefully() {
        ...
        let got = mqtt.handle_incoming_puback(&build_puback(101)).unwrap_err();
    ...
    fn incoming_pubrec_should_release_publish_from_queue_and_add_relid_to_rel_queue() {
        ...
        mqtt.handle_incoming_pubrec(&build_pubrec(2)).unwrap();
        assert_eq!(mqtt.inflight, 2);
    ...
    fn incoming_pubrec_should_send_release_to_network_and_nothing_to_user() {
        ...
        let packet = mqtt
            .handle_incoming_pubrec(&build_pubrec(1))
            .unwrap()
            .unwrap();
        match packet {
    ...
    fn incoming_pubrel_should_send_comp_to_network_and_nothing_to_user() {
        ...
        let packet = mqtt
            .handle_incoming_pubrel(&build_pubrel(1))
            .unwrap()
            .unwrap();
        match packet {
    ...
    fn incoming_pubcomp_should_release_correct_pkid_from_release_queue() {
        ...
        mqtt.outgoing_publish(publish).unwrap();
        mqtt.handle_incoming_pubrec(&build_pubrec(1)).unwrap();

        mqtt.handle_incoming_pubcomp(&build_pubcomp(1)).unwrap();
        assert_eq!(mqtt.inflight, 0);
    }
    ...
    fn outgoing_ping_handle_should_throw_errors_for_no_pingresp() {
        ...
        mqtt.handle_outgoing_packet(Request::Publish(publish))
            .unwrap();
        mqtt.handle_incoming_packet(Incoming::PubAck(build_puback(1)))
            .unwrap();
